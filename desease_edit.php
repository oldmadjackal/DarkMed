...
//--------------------------- Сохранение данных со страницы
  ...
  if(isset($put_id))
  {
       ...
       $res=$db->query($sql) ;
    if($res===false) {
             FileLog("ERROR", "Update DESEASES_REGISTRY... : ".$db->error) ;
                     $db->rollback();
                     $db->close() ;
        ...
        FileLog("", "Desease data saved successfully") ;
     SuccessMsg() ;
  }
...
//- - - - - - - - - - - - - - Удаление блоков
     if($delete!="") {
			$delete=                 substr($delete, 1) ;
                       ...
                       $sql="Delete from deseases_ext".
                            " Where desease_id='$put_id_'".
                            "  and  user      ='$user_'". 
                            "  and  order_num in ($delete)" ;
          $res=$db->query($sql) ;
       if($res===false) {
             FileLog("ERROR", "Delete DESEASES_EXT... : ".$db->error) ;
                     $db->rollback();
                     $db->close() ;
...
//- - - - - - - - - - - - - - Перенумерация блоков
	...
	foreach($orders_a as $order)
        { 
             ...
             $res=$db->query($sql) ;
          if($res===false) {
                FileLog("ERROR", "Update DESEASES_EXT(Order_num)... : ".$db->error) ;
                        $db->rollback();
                        $db->close() ;
...
//- - - - - - - - - - - - - - Сохранение файла картинки или вложения
    ...
    if(isset($_FILES[$image])) 
    {
        ...
        if($new_type=="Image") {  $file_type="Image" ;
         ...
         $path=LoadFile($image, $new_file, "deseases_registry", $put_id, $file_type, 
                             $actions, $spath, $error) ;
      if($path===false) {
...
//- - - - - - - - - - - - - - Сохранение данных блока
     ...
     {     
        ...
        if($new_file!="")  $sql.="       ,file='$new_file_', short_file='$new_sfile_'" ;
                           ...
                           $sql.=" Where desease_id= $put_id_".
                                 "  and  user      ='$user_'".
                                 "  and  id        = $ext_edit_" ;
     }
     else
     {
           $sql="Insert into deseases_ext".
                "       (desease_id, order_num, user, type, remark, file, short_file, www_link)".
                " Values('$put_id_','$new_order_','$user_','$new_type_','$new_remark_','$new_file_','$new_sfile_','$new_link_')" ;
     }

        $res=$db->query($sql) ;
     if($res===false) {
          FileLog("ERROR", "Insert/Update DESEASES_EXT... : ".$db->error) ;
                            $db->close() ;
     ...
     if(isset($ext_edit))   echo  "  document.getElementById('LiftUp_".$new_order."').focus() ;	\n" ;
        ...
        FileLog("", "Desease extension added successfully") ;
  }
//--------------------------- Создание новой записи
  ...
  if(!isset($get_id) and
     !isset($put_id)    ) 
  {
                       $sql="Insert into deseases_registry(type, user, name)".
                            " Values('dummy','$user_', '#$session_#')" ;

       $res=$db->query($sql) ;
    if($res===false) {
          FileLog("ERROR", "Insert DESEASES_REGISTRY... : ".$db->error) ;
                            $db->close() ;
         ErrorMsg("Ошибка на сервере. Повторите попытку позже.<br>Детали: ошибка создания карточки заболевания") ;
                         return ;
    }
       ...
       $res=$db->query($sql) ;
    if($res===false) {
          FileLog("ERROR", "Select id from DESEASES_REGISTRY... : ".$db->error) ;
                            $db->close() ;
         ErrorMsg("Ошибка на сервере. Повторите попытку позже.<br>Детали: ошибка идентификации карточки заболевания") ;
                         return ;
    }
        ...
        FileLog("", "New desease generated successfully") ;
  }
//--------------------------- Извлечение данных для отображения
  ...
  else
  {
       ...
       $res=$db->query($sql) ;
    if($res===false) {
          FileLog("ERROR", "Select * from DESEASES_REGISTRY... : ".$db->error) ;
                            $db->close() ;
...
//--------------------------- Извлечение списка категорий заболеваний
     ...
     $res=$db->query($sql) ;
  if($res===false) {
          FileLog("ERROR", "Select DESEASES_REGISTRY(TYPES)... : ".$db->error) ;
                            $db->close() ;
         ErrorMsg("Ошибка на сервере. Повторите попытку позже.<br>Детали: ошибка запроса справочника категорий заболеваний") ;
                         return ;
  }
  else
  {  
       echo "   a_types['dummy']='' ;				\n" ;
       echo "   a_types['0'    ]='Группа заболеваний' ;		\n" ;
...
//--------------------------- Извлечение дополнительных блоков
      ...
      echo     "  i_new_order.value='0' ;	\n" ;
			  ...
			  "  From deseases_ext e, doctor_page_main d".
			  " Where e.user      =d.owner".
                          "  and  e.desease_id='$get_id_'".
                          " Order by e.order_num" ;
     $res=$db->query($sql) ;
  if($res===false) {
          FileLog("ERROR", "Select DESEASES_EXT... : ".$db->error) ;
                            $db->close() ;
...
<title>DarkMed Desease Registry Card</title>
...
?>
  ...
  function SendFields() 
  {
	...
	error_text="" ;
     ...
     if(i_name.value==''     )  error_text="Название заболевания должно быть задано" ;
     if(i_type.value=='dummy')  error_text="Категория заболевания должна быть определена" ;
